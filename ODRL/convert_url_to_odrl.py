...
def main():
    parser = argparse.ArgumentParser(description="Convert policy descriptions from URLs to ODRL JSON-LD.")
    parser.add_argument("name", help="A short, unique name for the policy (e.g., 'copernicus').")
    parser.add_argument("--url", required=True, help="The URL to scrape the policy text from.")
    parser.add_argument("--outdir", default="custom-policies", help="Directory to save the resulting JSON-LD.")
    
    args = parser.parse_args()
    
    try:
        description_text = extract_text_from_url(args.url)
        # Optional: truncate if extremely long, or let the LLM context window handle it.
        # We will truncate to the first 5000 characters to prevent the model from failing.
        if len(description_text) > 5000:
            print("Warning: Extracted text is very long. Truncating to 5000 characters.")
